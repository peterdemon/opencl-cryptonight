...
uint32_t AESJobsReady = 0;
struct cryptonight_ctx *contexts[1024];
pthread_mutex_t WaitMutex = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t AddJobMutex = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t AESJobsCompleted = PTHREAD_COND_INITIALIZER;

void add_to_gpu_queue(struct cryptonight_ctx *ctx)
{
	pthread_mutex_lock(&AddJobMutex);
	contexts[AESJobsReady++] = ctx;
	pthread_mutex_unlock(&AddJobMutex);
	
	pthread_mutex_lock(&WaitMutex);
	pthread_cond_wait(&AESJobsCompleted, &WaitMutex);
}
...
void cpu_implementation1(struct cryptonight_ctx *ctx)
{
	uint32_t i, j;
	 __m128i *longoutput, *expkey, *xmminput;
	 
	longoutput = (__m128i *)ctx->long_state;
	expkey = (__m128i *)ctx->AESKey1;
...
void cpu_implementation2(struct cryptonight_ctx *ctx)
{
	uint32_t i;
...
void cpu_implementation3(struct cryptonight_ctx *ctx)
{
	uint32_t i, j;
	__m128i *longoutput, *expkey, *xmminput;
...
unsigned char do_gpu(void *output, uint8_t *input)
{
	...
	/*err = clEnqueueReadBuffer(cmdqueue, num, CL_TRUE, 0, sizeof(unsigned char), ret, 0, NULL, NULL);
	if(err != CL_SUCCESS)
	{
		printf("clEnqueueReadBuffer() call #2 returned %d!\n", err);
		exit(0);
	}*/
