...
void AES256Round(__local uint4 *aesoutput, ulong2 aesinput, uint4 ExpandedKey)
{
...
void AES256RoundInPlace(__local uint4 *aesinout, __constant uint *ExpandedKey)
{
...
/*void keccakf(ulong st[25], uint rounds)
{
    uint i, j, round;
    ulong t, bc[5];
...
int keccak(__global uchar *in, int inlen, __global uchar *md, int mdlen)
{
    ulong st[25];
    uchar temp[144];
    int i, rsiz, rsizw;
    ...
    for(i = 0; i < inlen; i++) temp[i] = in[i];
    
    temp[inlen++] = 1;
    //memset(temp + inlen, 0, rsiz - inlen);
    for(i = 0; i < (rsiz - inlen); i++) temp[inlen + i] = 0;
    ...
    //memcpy(md, st, mdlen);
    for(i = 0; i < mdlen; i++) md[i] = ((uchar *)st)[i];
...
__kernel void aes(__global uint4 *output, volatile __global uint4 *input, __constant uint *aeskey1, __constant uint *aeskey2, __global ulong2 *ina, __global ulong2 *inb, __global ulong *long_state)
{
	__local uint i, j, x, gid, lid;
	...
	for(i = 0; i < 0x40000; i++)
	{
		...
		//MulAddXor(&c, &a, &long_state[cindx]);
		hi= mad_hi(c.x, long_state[cindx], a.x);
		lo = (c.x * long_state[cindx]) + a.y;
		//hl = (ulong2)(c.x * long_state[cindx] + a.y, mad_hi(c.x, long_state[cindx], a.x)); 
		a.x = long_state[cindx] ^ hi;
		a.y = long_state[(cindx) + 1] ^ lo;
		//a.x = long_state[cindx] ^ hl.y;
		//a.y = long_state[cindx] ^ hl.x;
		long_state[cindx] = hi;
		long_state[(cindx) + 1] = lo;
		//long_state[cindx] = hl.y;
		//long_state[(cindx) + 1] = hl.x;
		...
		//MulAddXor(&b, &a, &long_state[bindx]);
